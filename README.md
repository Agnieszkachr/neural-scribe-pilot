...
**Project Type**: Feasibility Pilot for NCN Grant  
...
that requires larger sample size to confirm. The full NCN project will 
...
For this feasibility pilot, several methodological simplifications were made that will be upgraded in the full project:

1. **Standard vs. Dependence-Aware Inference**: Chunks were generated with a 150-word window and 75-word stride to maximize spatial resolution. While this 50% overlap violates the i.i.d. assumption required for the standard t-tests and confidence intervals used in this pilot (artificially inflating $n$), it was deemed acceptable for an initial feasibility check. As outlined in the proposal, the full project will resolve this by implementing dependence-aware inference (block permutation / hierarchical block bootstrap) to strictly account for structural overlap.
2. **Unweighted Effect Sizes**: Cohen's $d$ was calculated using a simple average of variances rather than a sample-size weighted pooled standard deviation. Given the drastic class imbalances (Paul $n=314$ vs Titus $n=8$), the full study will implement strictly weighted pooled estimators.
3. **Punctuation Removal**: Text was stripped of punctuation to match the classical stylometry (Burrows/PCA) baseline. While standard for word-frequency methods, removing punctuation degrades the syntactic signals that BERT-style models rely on. The full project will maintain Greek punctuation for the neural pipeline.
4. **Gradient approaching but not reaching significance** ($p = 0.077$) with 7 texts.
5. **Zero-shot only**: Fine-tuned models may reveal additional signal.

### Grant Justification

These results justify the NCN project:
...
├── visualize_dissociation.py # Dissociation comparison plot
...
---
*Pilot study for NCN grant application, 2026*
