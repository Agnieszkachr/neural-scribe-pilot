...
that requires larger sample size to confirm. Future work will 
...
Several methodological simplifications were made in this pilot:

1. **Standard vs. Dependence-Aware Inference**: Chunks were generated with a 150-word window and 75-word stride to maximize spatial resolution. While this 50% overlap violates the i.i.d. assumption required for the standard t-tests and confidence intervals (artificially inflating $n$), it was deemed acceptable for an initial feasibility check. Future work will resolve this by implementing dependence-aware inference (block permutation / hierarchical block bootstrap) to strictly account for structural overlap.
2. **Unweighted Effect Sizes**: Cohen's $d$ was calculated using a simple average of variances rather than a sample-size weighted pooled standard deviation. Given the drastic class imbalances (Paul $n=314$ vs Titus $n=8$), future work will implement strictly weighted pooled estimators.
3. **Punctuation Removal**: Text was stripped of punctuation to match the classical stylometry (Burrows/PCA) baseline. While standard for word-frequency methods, removing punctuation degrades the syntactic signals that BERT-style models rely on. Future work will maintain Greek punctuation for the neural pipeline.
